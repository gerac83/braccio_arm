 ...
 */
unsigned int _Braccio::begin(int soft_start_level, int speed, bool wait) {
...
int _Braccio::ServoMovement(int stepDelay, int vBase, int vShoulder, int vElbow,int vWrist_ver, int vWrist_rot, int vgripper, int speed[6]) {
  ...
  for(int i=0;i<6;i++){
	...
	int exit = 1;
  int c_base, c_shoulder, c_elbow, c_wristv, c_wristr, c_gripper;
	...
	while (exit)
	{
		...
		if (vBase != step_base)
		{
			base.write(step_base, speed[0], wait);
			//One step ahead
			if (vBase > step_base) {
				step_base++;
			}
		...
		{
			shoulder.write(step_shoulder, speed[1], wait);
			//One step ahead
			if (vShoulder > step_shoulder) {
				step_shoulder++;
			}
		...
		{
			elbow.write(step_elbow, speed[2], wait);
			//One step ahead
			if (vElbow > step_elbow) {
				step_elbow++;
			}
		...
		{
			wrist_ver.write(step_wrist_ver, speed[3], wait);
			//One step ahead
			if (vWrist_rot > step_wrist_ver) {
				step_wrist_ver++;
			}
    ...
    if (vWrist_ver != step_wrist_rot)
		{
			wrist_rot.write(step_wrist_rot, speed[4], wait);
			//One step ahead
			if (vWrist_ver > step_wrist_rot) {
				step_wrist_rot++;
			}
		...
		{
			gripper.write(step_gripper, speed[5], wait);
			if (vgripper > step_gripper) {
				step_gripper++;
			}
			//One step beyond
